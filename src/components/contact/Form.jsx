...
const commonClass =
  "input input-lg border-0 border-b-2 focus:outline-none placeholder:text-gray-400 focus:placeholder:text-picto-primary placeholder:text-[15px] md:placeholder:text-lg focus:border-picto-primary border-[#E6E8EB] w-full rounded-none px-0";
...
const Form = () => {
  ...
  return (
    <div>
      <p className="text-[12px] xs:text-[14px] max-lg:text-center sm:text-lg font-normal text-black">
        Feel free to fill out this form — it’s safe, reliable, and reaches me directly.
      </p>
      <div className="mx-2">
        <form ref={form} onSubmit={sendEmail} className="flex flex-col gap-4 mt-4">
          ...
          {status === "success" && (
            <p className="text-sm text-green-600 mt-2">Message sent — I will get back to you soon.</p>
          )}
          {status === "error" && (
            <p className="text-sm text-red-600 mt-2">Failed to send. Please try again or email me directly.</p>
          )}
        </form>
      </div>
    </div>
